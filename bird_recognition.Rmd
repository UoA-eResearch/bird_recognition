...
hist(audio_len_train[1,])
hist(audio_len_test[1,])
...
# Define the function
addResolution <- function(matrix) {
  test <- matrix(0,141,20)
  dimension <- dim(matrix)
  test[1:dimension[1], 1:dimension[2]] <- matrix
  test
}

# Add more elements to the matrices
trainMels_stretched <- lapply(trainMels, addResolution)
testMels_stretched <- lapply(testMels, addResolution)

# Draw spectrograms
dim(trainMels_stretched[[1]])
image(trainMels_stretched[[1]])
...
X_train = trainMels_stretched
Y_train <-  to_categorical(as.integer(train$birdid) -1)
X_test = testMels_stretched
