...
library(pbapply)
library(R.utils)
filtered_df = df[!is.na(df$true_length),]
pblapply(1:nrow(filtered_df), function(i) {
  wav = read_audio(filtered_df$filename[i])
  true_sr = round(length(wav) / filtered_df$true_length[i])
  print(paste(filtered_df$filename[i], true_sr))
  ...
  savewav(wav, true_sr, filename = output_filename)
})
```
