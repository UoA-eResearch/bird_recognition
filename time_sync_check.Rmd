...
read_BAR_metadata = function(files) {
  ...
  df = pblapply(files, function(f) {
    ...
    try({
      ...
      return(data.frame(filename = f, base_filename = basename(f), site = site, start = start, end = end, duration=duration, interval = interval(start, end), stringsAsFactors = FALSE))
    })
    NULL
  })
  df = do.call(rbind, df)
