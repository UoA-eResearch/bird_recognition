...
annotations$time_since_reset = rep(NA, nrow(annotations))
...
for (i in 1:nrow(annotations)) {
  if (substring(annotations$Individual.ID[i], 0, 2) == "PB") {
    ...
    annotations$time_since_reset[i] = time_length(annotations$datetime[i] - max(rfid$datetime[annotations$Nest.ID[i] == rfid$site & rfid$datetime < annotations$datetime[i] & str_detect(rfid$msg, "Reset")]))
...
ggplot(data = filtered_annotations, aes(x=Begin.Time..s., y=offset, color=filename)) +
    geom_point() +
...
```{r}
rfid$site == "NNS1" & rfid$msg == "PB0023.wav"
```
