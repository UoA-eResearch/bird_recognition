...
LG_threshold = 1
library(pbapply)
results = pblapply(1:sum(mask), function(i) {
  dt = processed_rfid$datetime[mask][i]
  this_site = processed_rfid$site[mask][i]
  bird = processed_rfid$msg[mask][i]
  pre_LG = filter(processed_rfid, site == this_site & datetime > dt - time_threshold  & datetime <= dt & msg == "Light Gate Change")
  post_LG = filter(processed_rfid, site == this_site & datetime < dt + time_threshold  & datetime >= dt & msg == "Light Gate Change")
  pre_int = sum(pre_LG$int)
  pre_ext = sum(pre_LG$ext)
  post_int = sum(post_LG$int)
  post_ext = sum(post_LG$ext)
  event_types = c()
  if (pre_ext - pre_int >= LG_threshold & post_int - post_ext >= LG_threshold) {
    #print(paste(i, bird, "entry", this_site, dt))
    event_types = c(event_types, "entry")
  } else if (pre_int - pre_ext >= LG_threshold & post_ext - post_int >= LG_threshold) {
    #print(paste(i, bird, "exit", this_site, dt))
    event_types = c(event_types, "exit")
  }
  if (length(event_types) >= 1) {
    return(data.frame(i = i, bird = bird, event_type = event_types, site = this_site, datetime = dt))
  }
})
